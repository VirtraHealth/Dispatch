...
export function LandingCTA({ variant }: LandingCTAProps) {
  ...
  if (variant === 'nav') {
    return (
      <button
        onClick={handleClick}
        className="text-sm font-semibold font-sans text-gray-700 hover:text-gray-900 transition-colors"
      >
  ...
  return (
    <button
      onClick={handleClick}
      className="inline-flex items-center gap-3 bg-ink text-white px-7 py-4 rounded-xl text-sm font-semibold font-sans hover:bg-gray-800 transition-colors shadow-sm"
    >
      {!session && (
      ...
      {session ? 'Go to dashboard →' : 'Connect Google Drive'}
    </button>
  )
}
