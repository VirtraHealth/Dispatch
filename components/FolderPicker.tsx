...
function buildPathMap(folders: DriveFolder[]): Map<string, string> {
  const idToName = new Map(folders.map(f => [f.id, f.name]))
  const paths = new Map<string, string>()
  for (const f of folders) {
    const parentName = f.parentId ? idToName.get(f.parentId) : null
    paths.set(f.id, parentName ? `${parentName} / ${f.name}` : f.name)
  }
  return paths
}
...
export function FolderPicker({ selected, onChange, max = 3 }: FolderPickerProps) {
  ...
  const loadFolders = useCallback(async (isRefresh = false) => {
    ...
    try {
      ...
      if (data.folders) {
        ...
        const msg: string = data.error || 'Failed to load folders'
        if (r.status === 401 || msg.toLowerCase().includes('token') || msg.toLowerCase().includes('unauthorized')) {
          setAuthError(true)
        }
        setError(msg)
      }
  ...
  if (error) {
    return (
      <div className="border border-red-100 bg-red-50 rounded-xl p-6 text-center space-y-3">
        <p className="text-sm text-red-600 font-sans font-medium">
          {authError ? 'Could not connect to Google Drive' : error}
        </p>
        {authError ? (
          <>
            <p className="text-xs text-red-400 font-sans">
              Your session may have expired. Sign out and sign back in to reconnect.
            </p>
            <a
              href="/api/auth/signout"
              className="inline-flex items-center gap-2 bg-red-600 text-white text-sm font-semibold font-sans px-4 py-2 rounded-lg hover:bg-red-700 transition-colors"
            >
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        {path !== folder.name && (
                          <div className="text-xs text-gray-400 truncate mt-0.5">{path}</div>
                        )}
                      </div>
                      {isSelected && (
